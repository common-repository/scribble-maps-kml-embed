...
(Kimili.Flash = function() {
	...
	return {
		...
		embed : function() {
			...
			tb_show('Scribble Maps Embed', url , false);
		}
...
(Kimili.Flash.Generator = function(){
	...
	var buildTag = function() {
		...
		$generator.find('input[type=text],input[id!="idType"]:checked,select').each(function(){
			...
			switch(this.name) {
				...
				case "idOrKml":
					switch($generator.find('input[name="idType"]:checked')[0].value)
					{
						case "mapID":
							tag += ' mapID="' + this.value + '"';
							break;
						case "kmlUrl":
							tag += ' kmlUrl="' + this.value + '"';
							break;
					}
					break;
				default:
