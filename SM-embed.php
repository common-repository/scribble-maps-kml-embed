...
/*
Plugin Name: Scribble Maps Embed
Plugin URI: http://www.scribblemaps.com
Description: Provides a full Wordpress interface for embedding maps created on ScribbleMaps.com
Version: 1.0.7
Author: ScribbleMaps
...
class ScribbleMapsEmbed
{
	...
	function ScribbleMapsEmbed()
	{
		// Register Hooks
		if (is_admin()) {
			
			// Default Options
			add_option('kml_flashembed_filename', 'http://www.scribblemaps.com/smwidget.swf');
			add_option('kml_flashembed_target_class', 'flashmovie');
			add_option('kml_flashembed_publish_method', '0');
			add_option('kml_flashembed_version_major', '9');
			...
			add_option('kml_flashembed_drag', 'true');
			add_option('kml_flashembed_position', 'true');
			add_option('kml_flashembed_mapType', 'true');
			add_option('kml_flashembed_zoom', 'true');
			add_option('kml_flashembed_legend', 'false');
			...
			wp_enqueue_script( 'scribblemapsembed', plugins_url('/scribble-maps-kml-embed/js/kfe.js'), array(), $this->version );
		...
		if ( get_option('kml_flashembed_reference_swfobject') == '1') {
			...
			if ( get_option('kml_flashembed_swfobject_source') == '0' ) {
				...
				wp_register_script( 'swfobject', plugins_url('/scribble-maps-kml-embed/js/swfobject.js'), array(), '2.2' );
			}
	...
	{
		$wp_myplugin_media_button = ' %s' . '<a id="add_scribblemap"' . ' onclick="Kimili.Flash.embed.apply(Kimili.Flash); return false;"'
			. ' class="thickbox"> <img src="' . plugins_url('/scribble-maps-kml-embed/img/logo13x13.png') . '" alt="Add Scribble Map"/></a>';
	...
	public static function getPluginUri() {
	    return get_option('siteurl') . '/wp-content/plugins/scribble-maps-kml-embed/';
	}
	...
	function processShortcode($code)
	{
		...
		$atts = $this->parseAtts($code[2]);
		$altContent = isset($code[4]) ? $code[4] : '';

		$attpairs	= preg_split('/\|/', $elements, -1, PREG_SPLIT_NO_EMPTY);
		
		$atts['movie'] = "http://widgets.scribblemaps.com/smwidget.swf";
		...
		$atts['maptype']	= (isset($atts['maptype'])) ? $atts['maptype'] : 'true';
		$atts['zoom']	= (isset($atts['zoom'])) ? $atts['zoom'] : 'true';
		$atts['drag']	= (isset($atts['drag'])) ? $atts['drag'] : 'true';
		$atts['position']	= (isset($atts['position'])) ? $atts['position'] : 'true';
		$atts['legend']	= (isset($atts['legend'])) ? $atts['legend'] : 'false';
		
		$atts['allowfullscreen'] = 'true';
		$atts['allowscriptaccess'] = 'always';
		
	
		$atts['fvars'] =  ((isset($atts['fvars'])) ? $atts['fvars'] . "&amp;" : "") . "mt=" . $atts['maptype'] . "&amp;z=" . $atts['zoom'] . "&amp;p=" . $atts['position'] . "&amp;d=" . $atts['drag'] . "&amp;l=" . $atts['legend'];
		if (isset($atts['mapid'])){
			$atts['fvars'] .= "&amp;id=" . $atts['mapid'];
		}
		if (isset($atts['kmlurl'])){
			$atts['fvars'] .= "&amp;kml=" . $atts['kmlurl'];
		}
	...
	function buildObjectTag($atts)
	{
		...
		*/
		...
		$out[] = '';
		$out[] = '<script type="text/javascript" src="http://widgets.scribblemaps.com/js/map/?width=' . $width . '&height=' . $height . '&' . $querystring . '"></script>';
		//$out[] = '<iframe width="'.$width.'" height="'.$height.'" src="http://widgets.scribblemaps.com/widget.html?' . $querystring . '" frameborder="0" style="border: none; margin: 0; padding: 0"></iframe>';
...
// <![CDATA[
	if (typeof Kimili !== 'undefined' && typeof Kimili.Flash !== 'undefined') {
		Kimili.Flash.configUrl = "<?php echo plugins_url('/scribble-maps-kml-embed/admin/config.php'); ?>";
	}
// ]]>	
</script>
<?php
	...
	{
		$buttonshtml = '<input type="button" class="ed_button" onclick="Kimili.Flash.embed.apply(Kimili.Flash); return false;" title="Embed a map from ScribbleMaps.com in your post" value="Scribble Maps Embed" />';
?>
<script type="text/javascript" charset="utf-8">
// <![CDATA[
	(function(){
...
add_action( (preg_match("/(\/\?feed=|\/feed)/i",$_SERVER['REQUEST_URI'])) ? 'template_redirect' : 'plugins_loaded', 'ScribbleMapsEmbed' );

function ScribbleMapsEmbed() {
	global $KimiliFlashEmbed;
	$KimiliFlashEmbed = new ScribbleMapsEmbed();
}
...
function kml_flashembed_plugin( $plugins ) {
	$plugins['kimiliflashembed'] = plugins_url('/scribble-maps-kml-embed/lib/tinymce3/editor_plugin.js');
	return $plugins;
}
